...
def mySetup(setup):
    # 标页面选第一标   # 2、首页选标；
    HomePage(setup).click_first_bid()
    bp = BidPage(setup)
    yield setup,bp  # setup就是driver对象。bp就是标页面对象
...
class TestInvest:
    ...
    # 正常场景 - 投资成功
    @pytest.mark.smoke
    def test_invest_success(self,mySetup):
        logging.info("******* 投资功能 - 正常场景用例：投资1000元成功。用户可用余额减少1000，标余额减少1000 *******")
        # 3、标页面获取用户余额；
        user_money_before_invest = mySetup[1].get_user_money()
        # 4、标页面获取 标的余额；
        bid_money_before_invest = mySetup[1].get_bid_left_money()
        # 5、标页面：投标动作；
        mySetup[1].invest(ID.success["money"])
        # 6、点击投标成功提示框当中：查看并激活按钮
        mySetup[1].click_activeButton_on_success_popup()
        # 7、个人页面：获取用户余额
        user_money_after_invest = UserPage(mySetup[0]).get_user_left_money()
        # 8、回退到标页面、刷新后：再次获取标的余额
        mySetup[0].back()
        mySetup[0].refresh()
        bid_money_after_invest = mySetup[1].get_bid_left_money()
        # 9、断言
